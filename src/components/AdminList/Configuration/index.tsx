...
const Configuration: React.FC = () => {
  ...
  const getColumnSearchPropsContact = (
    dataIndex: DataIndexContact
  ): ColumnType<DataTypeContact> => ({
  ...
  const getColumnSearchPropsAcademic = (
    dataIndex: DataIndexAcademic
  ): ColumnType<DataTypeAcademic> => ({
    filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters, close }) => (
      <div style={{ padding: 8 }} onKeyDown={(e) => e.stopPropagation()}>
        <Input
          ref={searchInput}
  ...
  return (
    <>
      {loading ? (
        ...
        <div>
          <Tabs defaultActiveKey="contact" id="uncontrolled-tab-example" className="mb-3" fill>
